...
class HeadPoseEstimationModel:
    ...
    def preprocess_output(self, outputs, image, face, facebox, print_flag=True, threshold = 0.5):
        ...
        yaw = outputs['angle_y_fc'].buffer[0][0]   # Axis of rotation: z
        pitch = outputs['angle_p_fc'].buffer[0][0] # Axis of rotation: y
        roll = outputs['angle_r_fc'].buffer[0][0] # Axis of rotation: x
        
        #Draw output
        if(print_flag):
            cv2.putText(image,"y:{:.1f}".format(yaw), (20,20), 0, 0.6, (255,255,255))
            cv2.putText(image,"p:{:.1f}".format(pitch), (20,40), 0, 0.6, (255,255,255))
            cv2.putText(image,"r:{:.1f}".format(roll), (20,60), 0, 0.6, (255,255,255))
