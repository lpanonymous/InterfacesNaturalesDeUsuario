...
class GazeEstimationModel:
    ...
    def preprocess_output(self, outputs, image,facebox, left_eye_point, right_eye_point,print_flag=True, threshold = 0.5):
        ...
        gaze_vector
        '''
        x = outputs.buffer[0][0]
        y = outputs.buffer[0][1]
        z = outputs.buffer[0][2]
        #Draw output
        if(print_flag):
            cv2.putText(image,"x:"+str('{:.1f}'.format(x*100))+",y:"+str('{:.1f}'.format(y*100))+",z:"+str('{:.1f}'.format(z)) , (20, 100), 0,0.6, (255,255,255), 1)
